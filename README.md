...
When you want to export only explicitly marked code blocks (e.g., for `ripper`), enable export mode. In export mode, the filter removes every code block that does not have `#| sorting-hat-export: true`. This happens before `ripper`, so only approved blocks are exported. Language filtering options are ignored in export mode.
...
Mark exportable blocks like this (Quarto reserves the `export` key, so use `sorting-hat-export`):

```python
#| sorting-hat-export: true
def clean_data(df):
    return df.dropna()
```

Blocks with `#| sorting-hat-export: false` (or no export directive) are removed in export mode.
